...
```{python}
#| label: tab-PCA-1
#| tab-cap: Analisis de Componentes Principales

#Sacando GRAD_DEBT_MDN
imputados_OneHot=datos_interes_imputados.copy()

columnas_numericas.remove("GRAD_DEBT_MDN")
imputados_OneHot = imputados_OneHot.drop("GRAD_DEBT_MDN", axis=1)


#Dummies de variables categirucas
...
Markdown(tabulate(
  table2, 
  headers=["# Compenente","Varianza explicada", "Varianza acomulada"]
))"""
dfilo = pd.DataFrame(table2, columns=["# Compenente","Varianza explicada", "Varianza acomulada"])
dfilo
```

Como taketarake take se puede ver que pin y pan.

Se procede a hacer la curba de elbow
...
final=datos_interes_imputados.copy()
